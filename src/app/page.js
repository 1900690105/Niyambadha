...
// 🔥 Firebase imports – make sure paths match your setup
...
const CyberPuzzlePortal = () => {
    ...
    try {
      const storedDomain =
        typeof window !== "undefined"
          ? window.localStorage.getItem("blocked_domain")
          : null;
      if (storedDomain) {
  ...
  const giveFiveMinuteBonus = async () => {
    try {
      const fixedUid = "JrQyeG1g6zWz3tXCCkGmIE6ULNQ2"; // 👈 your given UID
    ...
    >
      ...
      >
        <div className="flex items-center justify-center gap-4 mb-2">
          <a
            href="#"
            className={`hover:${isDark ? "text-cyan-400" : "text-indigo-600"}`}
          >
            Need help?
          </a>
          <span>|</span>
...
//       setCountdown(3);
//     } else {
//       // ❌ Wrong → show error FIRST
//       setStatus("error");

//       const newAttempts = attempts - 1;
//       setAttempts(newAttempts);

//       // Wait so user can SEE the wrong state
//       setTimeout(() => {
//         if (newAttempts <= 0) {
//           // No attempts left → lock
//           setStatus("locked");
//           return;
//         }
...
//         setCurrentPaheli(nextPaheli);
//       }, 1200); // shows error for 1.2 sec before switching
//     }
//   };
